...
public class Weather : MonoBehaviour
{
    ...
    [Header("API")]
    public string openWeatherAppID = "&appid = bbf3c99400e129ace6868f63e9d69cbc";
    //  public enum Units = new units;,
 ...
 //   }
    ...
    public IEnumerator SendRequest()
    {
        ...
        UnityWebRequest ipIdentifier = new UnityWebRequest("https://api.ipify.org?format=json");
        ipIdentifier.downloadHandler = new DownloadHandlerBuffer();
        yield return ipIdentifier.SendWebRequest();

        Debug.Log(ipIdentifier.downloadHandler.text);

        if (ipIdentifier.error == null || ipIdentifier.error == "")
        {
            var N = JSON.Parse(ipIdentifier.downloadHandler.text);
            currentIP = N["ip"].Value;
        }
        else
        {
            Debug.Log("WWW error: " + ipIdentifier.error);
        }

        Debug.Log(currentIP);

        UnityWebRequest cityRequest = new UnityWebRequest("http://www.geoplugin.net/json.gp?ip=" + currentIP); //get our location info
        cityRequest.downloadHandler = new DownloadHandlerBuffer();
        yield return cityRequest.SendWebRequest();

        Debug.Log(cityRequest.downloadHandler.text);
        ...
        UnityWebRequest request = new UnityWebRequest("http://api.openweathermap.org/data/2.5/weather?q=" + currentCity + openWeatherAppID + "&units=metric"); //get our weather
